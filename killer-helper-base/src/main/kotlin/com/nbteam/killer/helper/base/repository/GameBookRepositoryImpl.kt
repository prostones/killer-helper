...
@Repository
class GameBookRepositoryImpl : GameBookRepository {
    companion object {
        var gameBooks: MutableList<GameBook?> = ArrayList()
        ...
        init {
            ...
            gameBooks.add(
                GameBook(
                    ...
                    RoleEnum.LIE_REN,
                    RoleEnum.BAI_CHI,
                    RoleEnum.LAO_LIU_MANG,
                    RoleEnum.ZHA_DAN_REN
                )
            )
            ...
            gameBooks.add(
                GameBook(
                    "女|预|猎|白|摄|乌|丘|暗|噩|蚀时狼妃|狼巫|狼王",
                    RoleEnum.NV_WU,
                    RoleEnum.YU_YAN_JIA,
                    RoleEnum.LIE_REN,
                    RoleEnum.BAI_CHI,
                    RoleEnum.SHE_MENG_REN,
                    RoleEnum.WU_YA,
                    RoleEnum.QIU_BI_TE,
                    RoleEnum.AN_LIAN_ZHE,
                    RoleEnum.E_MENG_ZHI_YING,
                    RoleEnum.ZHU_SHI_LANG_FEI,
                    RoleEnum.LANG_WU,
                    RoleEnum.BAI_LANG_WANG
                )
            )
    ...
    override fun getRandom(playerTotal: Int?): GameBook? {
        val list = gameBooks.parallelStream().filter { book: GameBook? ->
            book!!.total.equals(
                playerTotal
            )
        }.collect(Collectors.toList())
        if (CollUtil.isEmpty(list)) {
            throw Exception("错误：不支持的玩家数量")
        }
        return list[RandomUtil.randomInt(0, list.size)]
    }

    @Throws(Exception::class)
    override fun getById(id: String?): GameBook? {
        return this.getById(id, null)
    }

    @Throws(Exception::class)
    override fun getById(id: String?, isBdModal: Boolean?): GameBook? {
        val realBook = gameBooks.stream().filter { book: GameBook? -> book!!.getId() == id }
            .findAny().orElseThrow {
                Exception(
                    String.format(
                        "错误：无法通过id[%s]找到板子",
        ...
        if (BooleanUtil.isTrue(isBdModal) && realBook!!.players.size == 9) {
            // 偷换玩家，如果模式多，可以再代理中实现
            val bdBook = BD_BOOK9
            bdBook.title = realBook.title
            return bdBook
        }
        if (BooleanUtil.isTrue(isBdModal) && realBook!!.players.size == 12) {
            // 偷换玩家，如果模式多，可以再代理中实现
            val bdBook = BD_BOOK
            bdBook.title = realBook.title
            return bdBook
    ...
    override fun list(): List<GameBook?>? {
        gameBooks.stream().forEach { book: GameBook? ->
            book!!.players.forEach(
                Consumer { p: Player -> p.number = 0 })
        }
        return gameBooks
    }
}