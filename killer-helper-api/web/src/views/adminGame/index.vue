<template>
  <div class="adminGame">
    ...
    <CellGroup inset v-for="player in detail.players">
      <Field :label="'座位号'+player.number" :error="player.role.goodMan == false"
             :label-class="player.role.goodMan == false? 'error-font':''"
...
async function create2(id: string) {
  const res = await createByGameBookId(id, isBdModal === '1')
  if (res.status === 500) {
